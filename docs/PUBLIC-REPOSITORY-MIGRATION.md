...
这些结果只能证明已检查的特征没有命中，不能证明代码绝对不存在任何业务秘密。公开前必须用 `tools/audit-public-readiness.sh` 重新扫描最终快照，并通过 `--extra-pattern-file` 从仓库外输入项目所有者掌握的服务器地址、令牌和历史凭据做精确复核。该文件不得提交、不得使用符号链接，审计结束后立即删除。

## 为什么不能直接把当前仓库改为 Public

基线审计发现过以下阻塞项；[Issue #252](https://github.com/DTB201/sb-user-manager/issues/252) 的公开源码快照已经在代码层处理前三项，但创建新仓库前仍需按最终提交重新验收：

- `src/` 和私有生成物仍包含 GitHub Token 输入、保存和私有 Release 下载路径。
- Release 同时发布私有版与分享版；公开后应收敛为一个匿名更新的公开版，避免继续保存无必要的长期 Token。
...
10. 准备仓库改名、首个公开 Release、旧客户端更新和失败回退的逐步操作清单，并再次取得明确授权。

## 建议切换顺序

以下步骤只能在项目所有者批准正式公开后执行：

1. 冻结当前私有仓库的 v4 变更，记录仓库 ID、默认分支、最新提交、Release 和设置；制作只读镜像备份。
2. 以临时名称创建空的公开仓库，先提交审计后的最小初始树。
3. 立即配置分支保护、Actions 权限、外部工作流审批和交互限制。
4. 通过公开 PR CI 和 `main` CI，创建高于现有正式版的首个公开 Release，并核对脚本、摘要和版本。
5. 验证旧配置中的 Token 会被兼容接受后丢弃，新公开版只通过匿名 HTTPS 查询和下载公开 Release。
6. 将当前私有仓库改名为内部档案名，再把已经验收的公开仓库改为正式名称。两个改名必须连续执行并立即核对远程地址。
7. 观察公开更新、Release 下载和 CI；没有异常后再更新 README、PROJECT 和正式来源记录。

## 回退方法

- 第 4 步之前：停止操作即可；当前私有仓库和现有服务器不受影响。
- 首个公开 Release 发布前：保留公开仓库但停止推广，修复后重新验收；不要删除后复用版本号。
- 仓库改名后、客户端切换前：可以把两个仓库恢复原名，但已经公开的源码不能视为已收回。
- 客户端开始使用公开更新源后：保持公开仓库和 URL 稳定，通过更高版本修复；不得依赖改回私有作为回退。
