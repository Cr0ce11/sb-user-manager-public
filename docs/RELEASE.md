...
3. 在测试机通过菜单执行一次更新。
4. 再次运行只读验收。
5. **核对 CHANGELOG 里每一条关于「升级之后会怎样」的断言**，它们只能由第 3 步那次真实菜单更新证实。
   这一条是 v4.25.27 用一次错误换来的：当时写了「升级到本版之后不用做任何事，两个目录会恢复成 755」，
   而**升级那一次是由旧脚本执行部署的**，跑的仍是那份把目录改坏的代码，新脚本里的修复要下一次才生效
   （[公开 Issue #252](https://github.com/Cr0ce11/sb-user-manager-public/issues/252)）。
   在新脚本上验证一个修复「有效」，不等于验证了「从上一版升上来之后有效」——**这两件事必须分别验**。
   拿不出第 3 步证据的断言，发布前就不要写进 CHANGELOG。
6. 记录已知限制和必要的回退版本。
