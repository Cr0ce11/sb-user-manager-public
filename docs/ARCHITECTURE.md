...
仓库退役任务不同时删除运行代码，避免把治理变更扩大成可能影响 standalone 的版本改造。专用代码、测试和 ADR 后续只按 [公开 Issue #17](https://github.com/DTB201/sb-user-manager-public/issues/17) 分阶段清理；每一步都必须证明现有菜单、状态 schema、迁移格式、事务顺序和服务器可见行为不变。
