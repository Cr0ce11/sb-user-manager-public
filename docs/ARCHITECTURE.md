...
[公开 Issue #17](https://github.com/DTB201/sb-user-manager-public/issues/17) 先移除角色选择和角色菜单分发，恢复唯一的 standalone 启动编排；随后删除全部不可达的控制器/落地源码、专属测试、POC 和 ADR 0005–0027。生成脚本只保留两个历史 helper 文件名的失败关闭墓碑，避免旧符号链接以普通文件名进入管理菜单。仓库清理不主动删除服务器上的历史 v5 文件；如未来发现此类残留，必须作为单独的破坏性清理任务取得服务器授权。
