...
v4.23.0 曾纳入一组未接入正式业务菜单的入口控制器与多落地休眠基础，但入口数据面、普通用户生命周期、共享配额和 v5 迁移从未发布。项目所有者现已永久终止该方向；这些模块不能作为可用产品能力或未来续作承诺。

仓库退役任务没有同时删除运行代码，避免把治理变更扩大成可能影响 standalone 的版本改造。[公开 Issue #17](https://github.com/DTB201/sb-user-manager-public/issues/17) 第一阶段先移除角色选择、入口/落地菜单分发与 helper 执行入口，恢复唯一的 standalone 启动编排；遗留 helper 文件名只会得到明确拒绝。其余低层专用代码、测试和 ADR 继续分批清理，每一步都必须证明现有菜单、状态 schema、迁移格式、事务顺序和服务器可见行为不变。
