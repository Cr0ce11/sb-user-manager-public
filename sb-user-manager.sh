...
#
# **兜底跟着一起退场，只是要晚一步。** LEGACY_MANAGER_DATA_DIR 与
# load_runtime_config 里那行 `:=` 只服务于项目所有者自己那几台还没搬的机器——
# 这个仓库虽然是公开的，但公开只是为了不受私有仓库的 CI 额度限制，唯一的使用者
# 就是他本人，没有别人的机器需要照顾。因此判断兜底还该不该留，只看他自己的机器
# 是不是都搬完了；都搬完之后，兜底和这个入口一起删。撤除时机的先后是唯一的差别：
# 入口跑完就撤，兜底要等最后一台机器搬完。
# ============================================================================
