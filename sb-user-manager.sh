...
build_merge_migration_payload() {
  ...
  0. 返回，不执行任何恢复
EOF
      prompt_migration_choice '请选择 [1]：' 1 '^[0-3]$' || return 1; choice="$MIGRATION_CHOICE"
      case "$choice" in
  ...
  0. 返回，不执行任何恢复
EOF
      prompt_migration_choice '请选择 [2]：' 2 '^[0-2]$' || return 1; choice="$MIGRATION_CHOICE"
      case "$choice" in
