...
| 发布形态 | `DTB201/sb-user-manager-public` 是唯一正式仓库；开发、Issue、Pull Request、完整 CI、匿名在线更新和不可变 Release 全部在这里完成 |
...
| 当前阶段任务 | 完成公开 [Issue #22](https://github.com/DTB201/sb-user-manager-public/issues/22) 的 v4.25.3 续期修复发布；随后按 [Issue #17](https://github.com/DTB201/sb-user-manager-public/issues/17) 独立清理未开放的休眠 v5 基础 |

现有最新完整正式版为 v4.25.2，主要功能包括安装、环境修复与完整卸载、原生 SS2022、既有 SS2022 + ShadowTLS 和 AnyTLS 用户管理、Nfuse 流量统计与配额、分流管理、可复用的预置出口与预置规则、共享预置运行配置、正式版与测试版 sing-box 切换、诊断报告、事务回滚以及加密迁移备份和恢复。同一用户最多可同时拥有一个原生 SS2022、一个既有 SS2022 + ShadowTLS 旧版入口和一个 AnyTLS，三类入口共享流量、配额、有效期和启停状态，并可按精确入口类型管理。v4.25.2 会在安装或更新改动系统前验证本机回环地址可用；v4.25.3 候选版将发布已合并的续期月数修复，正式发布前 v4.25.2 仍是当前 Release 和直接代码回退点。v4.23.0 纳入但从未开放的 v5 入口控制器和多落地基础不再作为产品方向；其清理必须通过独立版本证明不改变 standalone 行为。正式业务环境最后一次进入记录的统一版本是 v4.22.1，后续升级仍需独立授权和重新验收。
...
v4.18.1 已按原私有 Issue #96 完成首台正式服务器上线。上线前预检、固定 Release 全新部署、真实 AnyTLS 与 SS2022 + ShadowTLS 客户端 TCP/UDP、Nfuse 用量联动、上线后 30 分钟和 24 小时观察均已通过；最终 `release` 只读验收失败项为 0，观察期内没有服务异常重启或相关错误。首次上线任务已经关闭，项目进入阶段 3 的上线后维护。此后各服务器最后一次进入正式记录的统一状态为 v4.22.1 且运行良好；专用测试服务器已经弃用，v4.23.0 至 v4.25.2 仅采用本地门禁与 GitHub CI 验收，没有在本轮治理中登录或修改服务器。当前公开仓库没有已知未解决的 P0/P1 运行缺陷；Mihomo 导出候选需求已迁到公开 [Issue #16](https://github.com/DTB201/sb-user-manager-public/issues/16)，SEC-001 继续暂缓。后续正式环境变更仍需建立独立公开 Issue、准备回退方式并取得项目所有者明确授权。
