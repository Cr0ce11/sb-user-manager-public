...
| 测试环境 | 专用测试服务器已弃用；当前发布仅采用本地门禁与 GitHub CI 验收 |
| 正式环境 | 各服务器均已部署 v4.22.1 且运行良好；首次上线记录见 Issue #96（地址和凭据不进入仓库） |
| 发布形态 | 单一公开版；通过公开 GitHub Release 匿名更新 |
...
| 当前阶段任务 | [Issue #250](https://github.com/DTB201/sb-user-manager/issues/250) 建立公开仓库审计与迁移门禁，[Issue #252](https://github.com/DTB201/sb-user-manager/issues/252) 准备 MIT 许可的单一公开源码快照；不直接公开当前历史，创建公开仓库、改名和首个公开 Release 仍需另行授权；v5 开发继续暂停 |

现有最新完整正式版为 v4.23.0，主要功能包括安装、环境修复与完整卸载、SS2022 + ShadowTLS 和 AnyTLS 用户管理、Nfuse 流量统计与配额、分流管理、可复用的预置出口与预置规则、共享预置运行配置、正式版与测试版 sing-box 切换、诊断报告、事务回滚以及加密迁移备份和恢复。v4.23.0 在不改变 standalone 用户行为的前提下纳入休眠态 v5 入口控制器、多落地安全通道、原子 apply 与恢复、角色初始化和统一 readiness 门禁；入口数据面和多落地业务菜单仍未开放。v4.22.9 继续作为不可变代码回退版本；正式业务服务器继续运行 v4.22.1，后续升级仍需独立授权。
...
v4.18.1 已按 [Issue #96](https://github.com/DTB201/sb-user-manager/issues/96) 完成首台正式服务器上线。上线前预检、固定 Release 全新部署、真实 AnyTLS 与 SS2022 + ShadowTLS 客户端 TCP/UDP、Nfuse 用量联动、上线后 30 分钟和 24 小时观察均已通过；最终 `release` 只读验收失败项为 0，观察期内没有服务异常重启或相关错误。首次上线任务已经关闭，项目进入阶段 3 的上线后维护。此后各服务器均已更新到 v4.22.1 且运行良好；专用测试服务器已经弃用，v4.23.0 正式版仅采用本地门禁与 GitHub CI 验收。当前没有未解决的 P0/P1 问题；Issue #88 与 SEC-001 继续暂缓。后续正式环境变更仍需建立独立 Issue、准备回退方式并取得项目所有者明确授权。
