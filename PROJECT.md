...
| 当前阶段任务 | v4.25.30 已作为不可变 Release 发布（标签指向 `2f10635`，附件摘要 `7fd506bb…`）——本版新增「搬迁管理器数据目录」，把存量机器上管理器自己的数据（用户资料、内部备份、AnyTLS 自签证书）从 `/etc/sing-box` 搬到中立目录 `/etc/sb-user-manager`（[公开 Issue #276](https://github.com/Cr0ce11/sb-user-manager-public/issues/276)，[公开 Issue #172](https://github.com/Cr0ce11/sb-user-manager-public/issues/172) 三步走的第三步）。**这是一次性入口，正式机执行完就整段撤除**；老目录的兼容兜底等最后一台机器搬完之后一并删掉（`TODO.md` 的 ARCH-015）。项目所有者已实读确认五台正式机的 `MANAGER_DATA_DIR` 全部是 `/etc/sing-box`，即五台都需要搬。**待办**：五台正式机升级到本版后由项目所有者自行执行搬迁——升级本身不会搬动任何东西（已由 `sbm-mihomo` 的真实菜单更新证实：升级前后除版本号外一字不差）。**上一版的待确认项已关闭**：v4.25.29 那个 P1 的真实证据由 Air 升级后的自动修复给出，覆盖缺口也已由 `lifecycle` 新增的 geo 类别分流生命周期补上（[公开 Issue #270](https://github.com/Cr0ce11/sb-user-manager-public/issues/270)）|
