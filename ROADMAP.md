...
当前阶段：公开仓库最新正式版仍为不可变 [v4.25.2 Release](https://github.com/DTB201/sb-user-manager-public/releases/tag/v4.25.2)。v4.25.0 完成同一用户多协议共享账户能力，v4.25.1 修复不计量用户添加共享入口时的静默退出，v4.25.2 增加安装与更新前的本机回环地址预检；v4.25.3 候选版将发布续期月数被误当作数字时区的修复，待公开 PR、`main`、标签 CI 和 Release 保护全部通过后才成为正式版。本轮不登录或修改服务器，v4.25.2 保留为直接代码回退点。`DTB201/sb-user-manager-public` 是代码、Issue、Pull Request、完整 CI、匿名在线更新和 Release 的唯一正式来源；原私有仓库和双版本发布永久退役。正式业务环境最后一次进入记录的统一版本为 v4.22.1，升级需要独立公开 Issue、回退准备和明确授权；当前没有已知未解决的 P0 或 P1 运行缺陷。
...
原 v5 方向从未开放正式入口数据面或多落地业务菜单，现已由项目所有者永久终止，不再作为候选阶段或续作入口。自 v4.25.2 起进入公开版本的休眠基础不会在仓库退役任务中直接删除；后续只按公开 [Issue #17](https://github.com/DTB201/sb-user-manager-public/issues/17) 分阶段清理，并以 standalone 菜单、状态、迁移和运行行为不变为强制门禁。
