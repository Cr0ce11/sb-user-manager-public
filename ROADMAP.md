...
当前阶段：公开仓库最新正式版为不可变 [v4.25.5 Release](https://github.com/DTB201/sb-user-manager-public/releases/tag/v4.25.5)。v4.25.0 完成同一用户多协议共享账户能力，v4.25.1 修复不计量用户添加共享入口时的静默退出，v4.25.2 增加安装与更新前的本机回环地址预检，v4.25.3 修复续期月数被误当作数字时区的问题，v4.25.4 删除从未开放且已不可达的 v5 实现，v4.25.5 增加按月提前或延长用户有效期；本地门禁、公开 PR、`main`、发布保护预检、标签 CI、不可变 Release 和两项附件独立下载校验均已通过，本轮没有登录或修改服务器，v4.25.4 保留为直接代码回退点。`DTB201/sb-user-manager-public` 是代码、Issue、Pull Request、完整 CI、匿名在线更新和 Release 的唯一正式来源；原私有仓库和双版本发布永久退役。正式业务环境最后一次进入记录的统一版本为 v4.22.1，升级需要独立公开 Issue、回退准备和明确授权；当前没有已知未解决的 P0 或 P1 运行缺陷。
