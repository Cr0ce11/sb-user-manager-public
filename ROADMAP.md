...
当前阶段：公开仓库最新正式版为不可变 [v4.25.15 Release](https://github.com/Cr0ce11/sb-user-manager-public/releases/tag/v4.25.15)。v4.25.0 完成同一用户多协议共享账户能力，v4.25.1 修复不计量用户添加共享入口时的静默退出，v4.25.2 增加安装与更新前的本机回环地址预检，v4.25.3 修复续期月数被误当作数字时区的问题，v4.25.4 删除从未开放且已不可达的 v5 实现，v4.25.5 增加按月提前或延长用户有效期，v4.25.6 收拢代码审计缺陷修复与批量性能优化，v4.25.7 加固调度器命令目标门禁与更新锁语义并把脚本自身更新目标改为项目所有者的新 GitHub 用户名，v4.25.8 收拢一轮完整代码审查的缺陷修复与回归修正，v4.25.9 新增非交互只读查询入口，v4.25.10 修正只读状态的服务状态显示，v4.25.11 修复单元素 inbound 引起的一致性检查误报，v4.25.12 抽出内核适配层、把配置骨架收敛为单一来源并新增只读的骨架检查，v4.25.13 修复该检查把空数组误判为缺失导致的假警报，v4.25.14 让安装与更新在 TLS 瞬时失败时自动重试并收敛 GitHub 请求，v4.25.15 完成内核适配层收尾；本地门禁、公开 PR、`main`、发布保护预检、标签 CI、不可变 Release 和两项附件独立下载校验均已通过，本轮没有登录或修改正式服务器，v4.25.14 保留为直接代码回退点。`Cr0ce11/sb-user-manager-public` 是代码、Issue、Pull Request、完整 CI、匿名在线更新和 Release 的唯一正式来源；原私有仓库和双版本发布永久退役。正式业务环境已统一升级到 v4.25.11，由项目所有者执行并已确认一致性检查误报不再出现（[公开 Issue #120](https://github.com/Cr0ce11/sb-user-manager-public/issues/120)）；此前 v4.25.8 升级与迁移备份验证的记录见 [公开 Issue #100](https://github.com/Cr0ce11/sb-user-manager-public/issues/100)。后续升级仍需独立公开 Issue、回退准备和明确授权。当前没有已知未解决的 P0 或 P1 运行缺陷。
