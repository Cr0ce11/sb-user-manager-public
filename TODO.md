...
| MAINT-011 | P2 | 待办 | [公开 Issue #102](https://github.com/Cr0ce11/sb-user-manager-public/issues/102) | `check-shell-call-targets.py` 遇到看不懂的写法会静默停止检查后续代码 | 优先加「是否分析到文件末尾」的自检，其次把反向探针也放到文件靠前位置，最后再修分词器对 `$( )` 内双引号串中嵌套 `$(( ))` 的处理 |
| REL-011 | P1 | 进行中 | [公开 Issue #105](https://github.com/Cr0ce11/sb-user-manager-public/issues/105) | 准备并发布 v4.25.9 只读入口版本 | 待完成版本与生成物、本地门禁、PR、main、发布保护预检、标签 CI、两个 Release 附件摘要、标签源码一致性和不可变保护；不登录或修改服务器 |
