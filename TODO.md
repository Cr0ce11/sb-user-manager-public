...
| GOV-003 | P1 | 完成 | [Issue #252](https://github.com/DTB201/sb-user-manager/issues/252) | 生成单一公开版源码快照并验证匿名更新 | v4.23.1 已完成首个公开快照和匿名 Release，旧 Token 只兼容解析后丢弃；后续公开版本已按同一模型发布到 v4.25.5 |
...
| PERF-005 | P2 | 进行中 | [公开 Issue #52](https://github.com/DTB201/sb-user-manager-public/issues/52) | 批量生成与重建用户协议入口 | endpoint 校验、入站构造、受管 tag 与数组拼接一次处理；50 个三入口用户夹具验证调用数、耗时、逐字节配置、失败传播、生成物、本地门禁和公开 CI |
| PERF-006 | P2 | 进行中 | [公开 Issue #54](https://github.com/DTB201/sb-user-manager-public/issues/54) | 批量处理诊断报告脱敏 | 已排序脱敏表经标准输入交给单个 Python 进程；100 用户夹具验证调用数、耗时、逐字节输出、短名边界、元字符、子串、秘密 argv 和缺 Python 回退 |
