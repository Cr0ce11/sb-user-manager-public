...
grep -Fxq 'MANAGER_REPOSITORY="Cr0ce11/sb-user-manager-public"' "$ROOT/sb-user-manager.sh"
...
(
  ...
  curl() {
    ...
    jq -cn --arg digest "sha256:$(printf 'a%.0s' {1..64})" '{
      ...
      assets:[{
        ...
        browser_download_url:"https://github.com/Cr0ce11/sb-user-manager-public/releases/download/v9.9.9/sb-user-manager.sh",
        digest:$digest
      }]
    }'
  }
  ...
  grep -Fxq 'https://api.github.com/repos/Cr0ce11/sb-user-manager-public/releases/latest' \
    < <(tr '\0' '\n' < "$curl_args")
)
